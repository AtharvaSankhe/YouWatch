...
class _WatchState extends State<Watch> {
  ...
  void initState() {
    super.initState();
    setState(() {
      playerController = VideoPlayerController.networkUrl(Uri.parse(widget.video.videoUrl??""));
    });
                ...
                children: [
                  SizedBox(
                    width: double.infinity,
                      child: Center(child: Text(widget.video.title??"",style: const TextStyle(color: Colors.white,fontSize: 30),))),
                  const SizedBox(height: 15,),
                  Row(
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          child: Text(widget.video.userName??"",style: const TextStyle(color: Colors.black,fontSize: 25),)
                      ),
                      const Column(
                        children: [
                          Row(
                  ...
                  Text('Category: ${widget.video.category??""}',style: const TextStyle(color: Colors.white60),),
                  const SizedBox(height: 20,),
                  SizedBox(
                    width: double.infinity,
                    child: Text('Description: ${widget.video.description??""}',style: const TextStyle(color: Colors.white),),
                  ),
                ],
              ),
            ),
